...
function matchesSearch(item: CaseItem, query: string) {
	...
	const haystack = [
		...
		item.details || "",
		item.remarks || "",
		...Object.entries(item.facts).flat(),
	]
		.join(" ")
		.toLowerCase();
			...
			>
				...
				<Text fontSize="0.78rem" color="app.textDim" mt={2}>
					...
					{feed.lastSuccessfulFetchAt
						? ` • last successful fetch ${formatDate(feed.lastSuccessfulFetchAt)}`
						: ""}
				</Text>
				{!feed.fresh && feed.lastError && (
					<Text fontSize="0.78rem" color="app.error" mt={2}>
						Using the last known snapshot because the live source fetch failed:{" "}
						{feed.lastError}
					</Text>
				)}
			</Box>
